...
test('contest demo and modal controls remain usable on small screens and keyboards', () => {
  ...
  assert.match(readability, /\.workspace > \.point-panel \{ grid-column: 1; grid-row: 2; overflow: hidden; \}/)
  assert.match(readability, /\.workspace > \.point-panel \.compare-point-list \{[^}]*grid-auto-rows: max-content;[^}]*overflow-y: auto;/)
  assert.match(readability, /\.workspace > \.result-panel \{ grid-column: 3; grid-row: 1 \/ 3; \}/)
