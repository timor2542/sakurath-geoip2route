...
test('contest demo and modal controls remain usable on small screens and keyboards', () => {
  ...
  assert.equal((app.match(/@click="jumpToSidebar\('/g) || []).length, 5)
  assert.match(app, /target\.scrollIntoView\(\{ behavior: reducedMotion \? 'auto' : 'smooth', block: 'start' \}\)/)
  assert.match(readability, /\.sidebar-jump-buttons button \{[^}]*width: 44px;[^}]*height: 44px;[^}]*place-items: center;/)
  ...
  assert.equal((app.match(/class="refresh-arrowhead"/g) || []).length, 4)
