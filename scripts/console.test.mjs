...
test('contest demo and modal controls remain usable on small screens and keyboards', () => {
  ...
  assert.match(app, /class="sidebar-jump-nav"[^>]*:aria-label="t\('quickNavigation'\)"/)
  assert.equal((app.match(/@click="jumpToSidebar\('(?:current|update|add|points|demo)'\)"/g) || []).length, 5)
  assert.equal((app.match(/sidebarJumpTarget === '(?:current|update|add|points|demo)' \? 'location'/g) || []).length, 5)
  ...
  assert.match(readability, /\.sidebar-jump-list button \{[^}]*min-height: 33px;[^}]*font-family: inherit;/)
  assert.match(readability, /\.sidebar-jump-list button\.active i \{[^}]*rotate\(45deg\) scale\(1\)/)
  assert.match(readability, /\.control-panel > \.demo-launcher \{ scroll-margin-top: 292px; \}/)
