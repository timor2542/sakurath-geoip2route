...
test('contest demo and modal controls remain usable on small screens and keyboards', () => {
  ...
  assert.equal((app.match(/:aria-label="t\('addIpToList'\)" :title="t\('addIpToList'\)"/g) || []).length, 1)
  ...
  assert.match(app, /<aside class="point-panel panel-card"/)
  assert.match(app, /<aside class="point-panel panel-card"[\s\S]*class="panel-section compare-add-section point-panel-add-section"[\s\S]*<\/aside>/)
  assert.equal((app.match(/class="panel-section compare-add-section/g) || []).length, 1)
  assert.match(readability, /\.workspace > \.control-panel \{ grid-column: 1; grid-row: 1; \}/)
  assert.match(readability, /\.workspace > \.point-panel \{ grid-column: 1; grid-row: 2 \/ 4; display: flex; flex-direction: column; overflow: hidden; \}/)
  assert.match(readability, /\.workspace > \.point-panel \.compare-point-list \{[^}]*grid-auto-rows: max-content;[^}]*overflow-y: auto;/)
  assert.match(readability, /\.workspace > \.result-panel \{ grid-column: 3; grid-row: 1 \/ 4; \}/)
  assert.match(readability, /\.workspace > \.activity-console,[\s\S]*grid-column: 2;[\s\S]*grid-row: 3;/)
