...
test('contest demo and modal controls remain usable on small screens and keyboards', () => {
  ...
  assert.equal((app.match(/@click="jumpToSidebar\('(?:current|update|add|points|demo)'\)"/g) || []).length, 10)
  assert.equal((app.match(/sidebarJumpTarget === '(?:current|update|add|points|demo)' \? 'location'/g) || []).length, 10)
  ...
  assert.match(readability, /\.control-panel > \.demo-launcher \{ scroll-margin-top: 16px; \}/)
