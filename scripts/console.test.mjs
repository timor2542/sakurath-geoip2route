...
test('contest demo and modal controls remain usable on small screens and keyboards', () => {
  ...
  assert.match(app, /<select id="sidebar-jump-select" v-model="sidebarJumpTarget" @change="jumpToSidebar\(sidebarJumpTarget\)"/)
  assert.equal((app.match(/<option value="(?:current|update|add|points|demo)"/g) || []).length, 5)
  assert.match(app, /target\.scrollIntoView\(\{ behavior: reducedMotion \? 'auto' : 'smooth', block: 'start' \}\)/)
  assert.match(readability, /\.sidebar-jump-nav select \{[^}]*min-height: 44px;[^}]*font-family: inherit;/)
  assert.match(readability, /\.control-panel > \.demo-launcher \{ scroll-margin-top: 178px; \}/)
