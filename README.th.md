...
ศึกษาสมการ ข้อสมมติ และข้อจำกัดทั้งหมดได้ที่ [ALGORITHM.md](docs/ALGORITHM.md)
...
| `docs/` | เอกสารอัลกอริทึม การตรวจสอบ และรายการตรวจสอบก่อนเผยแพร่ |

## การเผยแพร่บน Netlify

1. ส่งซอร์สโค้ดขึ้น GitHub โดยไม่รวม `.env.local`, `.env`, `node_modules` และข้อมูลลับ
2. เชื่อมต่อคลังซอร์สโค้ดกับ Netlify
3. เพิ่มตัวแปรสภาพแวดล้อมชื่อ `IP2LOCATION_API_KEY`
4. เผยแพร่ด้วยค่าที่กำหนดไว้ใน `netlify.toml`
...
ฟังก์ชันแบบไร้เซิร์ฟเวอร์จำกัดอัตราการเรียกใช้ต่อผู้ใช้และโดเมน โดยค้นหาข้อมูลได้ 60 ครั้งต่อนาที และตรวจหา IP ปัจจุบันได้ 20 ครั้งต่อนาที ระบบเก็บผลลัพธ์ที่สำเร็จไว้ชั่วคราว 15 นาที สูงสุด 500 รายการต่ออินสแตนซ์ที่ยังทำงานอยู่ ควรตรวจสอบบันทึกการเผยแพร่ว่า Netlify เปิดใช้ข้อจำกัดทั้งสองรายการ และติดตามโควตา IP2Location หลังเปิดให้บริการ

บริการโฮสต์เว็บไซต์แบบคงที่ เช่น GitHub Pages เพียงอย่างเดียว ไม่สามารถเรียกใช้ฟังก์ชันค้นหาข้อมูลที่รวมอยู่ในโครงการนี้ได้
...
ปัจจุบันชุดทดสอบมีการทดสอบ 45 รายการ ครอบคลุมการจัดอันดับ การนำเข้า CSV การรวม IPv6 ที่ซ้ำกัน การตรวจสอบ IP ปัจจุบัน ความคืบหน้าของ API การเข้าถึง การปิดบังข้อมูลลับ แคช และการจำกัดอัตราการเรียกใช้

การทดสอบอัตโนมัติใช้ข้อมูลจำลองสำหรับ HTTP, DNS และ API ต้นทาง จึงยังต้องตรวจสอบ API จริง เว็บไซต์ที่เผยแพร่ต่อสาธารณะ เค้าโครงจริง และการทำงานของข้อจำกัดอัตราการเรียกใช้บน Netlify ด้วยตนเองก่อนส่งผลงาน
