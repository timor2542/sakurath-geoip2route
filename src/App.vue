<template>
  <div class="app-shell" :data-theme="resolvedTheme">
    ...
    <main class="workspace" :class="{ 'console-open': consoleOpen }">
      <aside class="section-rail" :class="{ expanded: sectionRailExpanded }" :aria-label="t('quickNavigation')" @keydown.esc.stop.prevent="sectionRailExpanded = false">
        <button class="section-rail-toggle" type="button" aria-controls="section-rail-panel" :aria-expanded="sectionRailExpanded" :aria-label="t(sectionRailExpanded ? 'collapseNavigation' : 'expandNavigation')" :title="t(sectionRailExpanded ? 'collapseNavigation' : 'expandNavigation')" @click="sectionRailExpanded = !sectionRailExpanded">
          <svg class="section-rail-menu-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M9 6h10M9 12h10M9 18h10"/><circle cx="5" cy="6" r="1"/><circle cx="5" cy="12" r="1"/><circle cx="5" cy="18" r="1"/></svg>
          <span class="section-rail-toggle-label">{{ t('sections') }}</span>
          <svg class="section-rail-chevron" viewBox="0 0 24 24" aria-hidden="true"><path d="m14 7-5 5 5 5"/></svg>
        </button>
        <nav id="section-rail-panel" class="section-rail-nav" :aria-label="t('quickNavigation')">
          <button type="button" :class="{ active: sidebarJumpTarget === 'current' }" :aria-current="sidebarJumpTarget === 'current' ? 'location' : undefined" :aria-label="t('addCurrentIp')" :title="t('addCurrentIp')" @click="jumpToSidebar('current')">
          ...
          <button type="button" :class="{ active: sidebarJumpTarget === 'points' }" :aria-current="sidebarJumpTarget === 'points' ? 'location' : undefined" :aria-label="appMode === 'ranking' ? t('allIpPoints') : t('mapPoints')" :title="appMode === 'ranking' ? t('allIpPoints') : t('mapPoints')" @click="jumpToSidebar('points')">
            <span class="section-rail-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M12 21s6-5.1 6-11a6 6 0 1 0-12 0c0 5.9 6 11 6 11Z"/><circle cx="12" cy="10" r="2"/></svg></span><span class="section-rail-label">{{ appMode === 'ranking' ? t('allIpPoints') : t('mapPoints') }}</span>
          </button>
          <button type="button" :class="{ active: sidebarJumpTarget === 'demo' }" :aria-current="sidebarJumpTarget === 'demo' ? 'location' : undefined" :aria-label="t('demoTools')" :title="t('demoTools')" @click="jumpToSidebar('demo')">
            <span class="section-rail-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><rect x="3" y="5" width="18" height="14" rx="3"/><path d="m10 9 5 3-5 3Z"/></svg></span><span class="section-rail-label">{{ t('demoTools') }}</span>
          </button>
        </nav>
      </aside>
      <MapCanvas
...
const translations = {
  ...
  th: {
    expandNavigation:'ขยายเมนูนำทาง',collapseNavigation:'ย่อเมนูนำทาง',
...
const sectionRailExpanded = ref(false)
