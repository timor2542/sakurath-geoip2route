    ...
    <main class="workspace" :class="{ 'console-open': consoleOpen }">
      ...
      <aside class="control-panel panel-card">
        ...
        <template v-if="appMode === 'ranking'">
          <section class="panel-section ranking-intro">
            ...
            <div v-if="comparePoints.length" class="compare-point-list">
              <div v-for="point in comparePoints" :key="point.id" :class="['compare-point-row', { selected: rankingSourceId === point.id }]">
                <button class="compare-point-select" type="button" :disabled="measurementBusy || rankingBasis === 'demo'" @click="rankingSourceId = point.id">
                  ...
                  <span class="source-chip">{{ point.demoGroup ? t('simulated') : point.source === 'ip2location' ? 'LIVE' : 'SAMPLE' }}</span>
                </button>
        ...
        <template v-else>
          <section class="panel-section compare-intro">
            <div class="section-heading">
            ...
            <div v-if="comparePoints.length" class="compare-point-list">
              <div v-for="(point, index) in comparePoints" :key="point.id" :class="['compare-point-row', { selected: selectedCompareIds.includes(point.id) }]">
                <button class="compare-point-select" type="button" @click="selectComparePoint(point.id)">
                  ...
                  <span class="source-chip">{{ point.source === 'ip2location' ? 'LIVE' : 'SAMPLE' }}</span>
                </button>
...
const translations = {
  ...
  th: {
    compareMenuHint:'เทียบสองจุด',serverRanking:'จัดอันดับเซิร์ฟเวอร์',rankingMenuHint:'จัดอันดับทุกจุด',rankAllServers:'จัดอันดับ IP ทั้งหมด',rankingIntro:'เลือก IP หนึ่งรายการเป็นต้นทาง แล้ว IP อื่นทั้งหมดในรายการเดียวกันจะกลายเป็นเซิร์ฟเวอร์สำหรับจัดอันดับโดยอัตโนมัติ',sourceIp:'IP ต้นทาง',rankingFormulaHint:'คะแนนเส้นทางใช้ Latency ที่วัดจากเบราว์เซอร์เมื่อมี Probe URL หากไม่มีจะใช้ระยะทางเป็นสัญญาณสำรองอย่างโปร่งใส',addIpToList:'เพิ่ม IP ลงรายการ',pasteList:'วางรายการ',importCsv:'นำเข้า CSV',allIpPoints:'IP ทั้งหมด',chooseSource:'เลือก IP ต้นทาง',source:'ต้นทาง',recommendedServer:'เซิร์ฟเวอร์ที่แนะนำ',bestGeoFit:'เหมาะสมทางภูมิศาสตร์',geoFitScore:'คะแนน Geo fit',geoFitExplanation:'ประเมินจากระยะทางภูมิศาสตร์และประเภทการใช้งานเครือข่าย',fromSource:'จาก IP ต้นทาง',allAvailableServers:'เซิร์ฟเวอร์ทั้งหมด',selectedServer:'เซิร์ฟเวอร์ที่เลือก',needTwoIps:'เพิ่ม IP อย่างน้อยสองรายการ',needTwoIpsHint:'หนึ่ง IP เป็นต้นทาง และ IP ที่เหลือจะถูกจัดอันดับเป็นเซิร์ฟเวอร์',rankingAccuracyNote:'Latency จากเบราว์เซอร์เป็นค่ากลางจาก HTTP 3 ครั้ง และรวมเวลา DNS, TLS และการตอบสนองของเซิร์ฟเวอร์ ไม่ใช่ ICMP Ping หรือ Traceroute',bulkIpImport:'นำเข้า IP หลายรายการ',pasteIpList:'วางรายการ IP หรือ Hostname',onePerLine:'หนึ่ง IP หรือ Hostname ต่อหนึ่งบรรทัด',bulkImportNote:'รองรับรายการทั่วไปและ CSV ที่มีคอลัมน์ target/ip กับ probe_url โดยจะข้าม IP ที่มีอยู่แล้ว',addAllToMap:'ค้นหาและเพิ่มทั้งหมด',bulkComplete:'เพิ่ม IP ทั้งหมดลงแผนที่แล้ว',bulkPartial:'บางรายการไม่สามารถค้นหาตำแหน่งได้',noValidTargets:'ไม่พบ IP หรือ Hostname ที่ถูกต้องในไฟล์นี้',language:'ภาษา',deleteIp:'ลบ IP',ipRemoved:'ลบ IP ออกจากแผนที่แล้ว',noSampleIps:'ไม่มี IP ตัวอย่างเหลือในรายการ',workspaceMode:'โหมดการใช้งาน',ipCompare:'เปรียบเทียบ IP',ipMap:'แผนที่ IP',compareAnyTwo:'เปรียบเทียบ IP สองจุด',compareIntro:'IP ทุกจุดถูกปักบนแผนที่ เลือกหมุดหรือรายการสองจุดเพื่อเปรียบเทียบกัน',updating:'กำลังอัปเดต',updateFromIp2Location:'อัปเดตรายการจาก IP2Location',addIpToMap:'เพิ่ม IP ลงแผนที่',mapPoints:'จุดบนแผนที่',selectTwoPoints:'เลือกจุด A และ B',clear:'ล้าง',ipComparison:'เปรียบเทียบ IP',chooseAAndB:'เลือกจุด A และ B',comparisonReady:'IP ที่เลือกถูกเชื่อมบนแผนที่และแสดงข้อมูลเปรียบเทียบด้านล่าง',comparisonEmpty:'เลือกจุดบนแผนที่สองจุดเพื่อเริ่มเปรียบเทียบ',selectPointA:'เลือกจุดแรก',selectPointB:'เลือกจุดที่สอง',greatCircleDistance:'ระยะทางบนผิวโลก',field:'ข้อมูล',country:'ประเทศ',regionCity:'ภูมิภาค / เมือง',timezone:'เขตเวลา',sameCountry:'ประเทศเดียวกัน',differentCountry:'คนละประเทศ',sameAsn:'ASN เดียวกัน',differentAsn:'คนละ ASN',sameNetworkType:'ประเภทเครือข่ายเดียวกัน',differentNetworkType:'คนละประเภทเครือข่าย',pickTwoFromMap:'เลือกสองจุดจากแผนที่',pickTwoHint:'จุดแรกจะเป็น A และจุดที่สองเป็น B หากเลือกจุดใหม่ ระบบจะแทนที่ B',compareAccuracyNote:'ตำแหน่งมาจาก IP2Location และเป็นค่าประมาณ โดย Anycast IP อาจระบุตำแหน่งเครือข่ายต่างกันได้',usingLiveList:'รายการอัปเดตด้วย IP2Location แล้ว',usingSampleList:'กำลังใช้ตำแหน่งตัวอย่าง กดอัปเดตเมื่อ Live API พร้อม',updateComplete:'อัปเดตรายการ IP จาก IP2Location แล้ว',partialUpdate:'IP บางรายการใช้ตำแหน่งสำรอง เพราะ Live API ไม่ส่งข้อมูลกลับมา',addedToMap:'เพิ่ม IP ลงแผนที่แล้ว',measureAvailable:'วัด Probe ที่พร้อม',measuringLatency:'กำลังวัด',bestRouteMatch:'เส้นทางที่เหมาะสมที่สุด',routeScoreExplanation:'รวม Latency ระยะทาง และความเหมาะสมของเครือข่าย พร้อมแสดงที่มาของคะแนน',browserMeasured:'ค่ากลางจาก HTTP 3 ครั้ง',distanceFallback:'ใช้ระยะทางเป็นค่าทดแทน',confidence:'ความน่าเชื่อถือ',estimated:'ประเมิน',threeSamples:'วัดสำเร็จ 3 ครั้ง',noProbeYet:'เพิ่ม Probe URL เพื่อวัดจริง',latencySignal:'สัญญาณ Latency',whyThisServer:'เหตุผลที่เซิร์ฟเวอร์นี้ได้อันดับหนึ่ง',probeUrl:'Probe URL (ไม่บังคับ)',measureNow:'วัดตอนนี้',probeHelp:'ใช้ URL ของ Health endpoint หรือไฟล์ขนาดเล็กจากเซิร์ฟเวอร์นี้ เวลาที่ได้รวมการเชื่อมต่อและการตอบสนองของเซิร์ฟเวอร์',probeSaved:'วัด Probe และอัปเดตอันดับแล้ว',probeFailed:'เบราว์เซอร์นี้ไม่สามารถเข้าถึง Probe ได้',unreachable:'เข้าถึงไม่ได้',noProbes:'กรุณาเพิ่ม Probe URL อย่างน้อยหนึ่งรายการ',measurementComplete:'วัด Probe ที่พร้อมทั้งหมดแล้ว',measurementPartial:'ไม่สามารถเข้าถึง Probe บางรายการได้',
    brandNote:'เปรียบเทียบ IP และจัดอันดับเซิร์ฟเวอร์ทั่วโลก',liveGeo:'ข้อมูล IP2LOCATION จริง',demoGeo:'ข้อมูลตัวอย่าง',refresh:'รีเฟรช',help:'วิธีใช้',theme:'ธีม',closeDialog:'ปิดหน้าต่าง',clientLocation:'ตำแหน่งของผู้ใช้',unknownCity:'ไม่ทราบเมือง',copyIp:'คัดลอก IP',testAnotherIp:'ทดลอง Client IP หรือ Hostname อื่น',ipPlaceholder:'8.8.8.8 หรือ example.com',quickSample:'ตัวอย่างประเทศและ IP',chooseCountryIp:'เลือกประเทศและ IP ตัวอย่าง',sampleIpNote:'โหมด Live จะค้นหา IP ที่เลือกด้วย IP2Location ผลของ Anycast อาจเปลี่ยนแปลงได้',scenario:'สถานการณ์',chooseUseCase:'เลือกกรณีใช้งาน',addEndpoint:'เพิ่ม Endpoint',endpoints:'ENDPOINTS',candidates:'ตัวเลือก',testingEndpoints:'กำลังทดสอบ Endpoint…',runAnalysis:'วิเคราะห์เส้นทาง',recommendedEndpoint:'ENDPOINT ที่แนะนำ',bestMatch:'เหมาะสมที่สุด',routeScore:'คะแนนเส้นทาง',scoreExplanation:'รวมสถานะ ความหน่วง ระยะทาง และประเภทเครือข่าย',latency:'ความหน่วง',distance:'ระยะทาง',health:'สถานะ',network:'เครือข่าย',measured:'วัดจริง',demoEstimate:'ค่าตัวอย่าง',greatCircle:'ระยะทางบนผิวโลก',endpointStatus:'สถานะ Endpoint',usageType:'ประเภทการใช้งาน IP',routeRanking:'อันดับเส้นทาง',allCandidates:'Endpoint ทั้งหมด',simulateFailover:'จำลองเซิร์ฟเวอร์ล่ม',selectedEndpoint:'ENDPOINT ที่เลือก',location:'ตำแหน่ง',score:'คะแนน',exportJson:'ส่งออก JSON',exportCsv:'ส่งออก CSV',accuracyNote:'ระยะทางภูมิศาสตร์เป็นค่าประมาณและไม่รับประกันความเร็วเครือข่าย ระบบจึงให้น้ำหนักกับค่าที่วัดจากเบราว์เซอร์มากกว่า',customEndpoint:'ENDPOINT กำหนดเอง',endpointName:'ชื่อ Endpoint',hostOrIp:'Hostname หรือ Public IP',probeNote:'หากไม่มี Probe URL ระบบยังค้น GeoIP ได้ แต่จะแสดงความหน่วงเป็นค่าประมาณ',locating:'กำลังค้นหา…',locateAndAdd:'ค้นหาและเพิ่ม Endpoint',quickGuide:'คู่มือฉบับย่อ',howItWorks:'GeoIP2Route ทำงานอย่างไร',helpLocate:'เพิ่ม IP',helpLocateDesc:'เพิ่ม IP เดี่ยว วางหลายรายการ หรือนำเข้า CSV โดย IP2Location จะให้ตำแหน่ง ISP, ASN และประเภทเครือข่าย',helpMeasure:'เปรียบเทียบ',helpMeasureDesc:'เลือก IP สองจุดเพื่อดูความแตกต่างด้านภูมิศาสตร์และเครือข่ายบนแผนที่',helpRecommend:'จัดอันดับและตรวจสอบ',helpRecommendDesc:'เลือกต้นทาง เพิ่ม Probe URL หากมี วัด Latency จากเบราว์เซอร์ และดูที่มาของคะแนนทุกส่วน',helpCaveat:'ตำแหน่ง IP เป็นค่าประมาณ ไม่ใช่ GPS และเวลา HTTP รวมภาระจากเบราว์เซอร์กับเซิร์ฟเวอร์ ระบบจึงแยกค่าที่วัดจริงกับค่าประเมินอย่างชัดเจน',gotIt:'เข้าใจแล้ว',excellent:'ยอดเยี่ยม',good:'ดี',fair:'พอใช้',poor:'ต่ำ',online:'ออนไลน์',degraded:'มีปัญหาบางส่วน',offline:'ออฟไลน์',testing:'กำลังทดสอบ'
  }
}
...
function scoreReason(server) {
  ...
  const explanation = language.value === 'th'
    ? `${main} ให้ ${contribution} คะแนน + เครือข่าย ${network} คะแนน = ${server.score.toFixed(1)}`
    : `${main} contributes ${contribution} points + network ${network} points = ${server.score.toFixed(1)}.`
  return explanation + (gap === null ? ` ${t('onlyEligible')}` : language.value === 'th' ? ` สูงกว่าอันดับ 2 อยู่ ${gap} คะแนน` : ` Lead over #2: ${gap} points.`)
}
...
async function loadRankingDemo() {
  ...
  comparePoints.value.push(...rankingDemo.filter(point => !ids.has(point.id)).map(point => structuredClone(point)))
...
watch(language, value => {
  ...
  comparePoints.value = comparePoints.value.map(point => {
    ...
    return sample && point.source !== 'ip2location'
      ? { ...point, country_name: sample.country[value], city_name: sample.city[value] }
      : point
  })
})
