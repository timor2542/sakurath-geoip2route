      ...
      <aside class="control-panel panel-card">
        <div class="sidebar-sticky-header">
          ...
          <nav class="sidebar-jump-nav" :aria-label="t('quickNavigation')">
            <span>{{ t('jumpTo') }}</span>
            <div class="sidebar-jump-buttons">
              <button type="button" @click="jumpToSidebar('current')" :aria-label="t('addCurrentIp')" :title="t('addCurrentIp')"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/><path d="M12 2v4m0 12v4M2 12h4m12 0h4"/></svg></button>
              <button type="button" @click="jumpToSidebar('update')" :aria-label="t('updateFromIp2Location')" :title="t('updateFromIp2Location')"><svg class="refresh-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M20.25 12a8.25 8.25 0 1 1-2.42-5.83"/><path class="refresh-arrowhead" d="m21.3 9.1-7-.95 4.15-5.7Z"/></svg></button>
              <button type="button" @click="jumpToSidebar('add')" :aria-label="t('addIpToList')" :title="t('addIpToList')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg></button>
              <button type="button" @click="jumpToSidebar('points')" :aria-label="appMode === 'ranking' ? t('allIpPoints') : t('mapPoints')" :title="appMode === 'ranking' ? t('allIpPoints') : t('mapPoints')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 21s7-5.1 7-12a7 7 0 1 0-14 0c0 6.9 7 12 7 12Z"/><circle cx="12" cy="9" r="2"/></svg></button>
              <button type="button" @click="jumpToSidebar('demo')" :aria-label="t('tryRankingDemo')" :title="t('tryRankingDemo')"><svg class="jump-demo-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="m9 6 9 6-9 6Z"/></svg></button>
            </div>
          </nav>
        </div>
